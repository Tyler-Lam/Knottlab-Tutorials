...
for slide in slides:
    ...
    f_tiff = f'/common/knottsilab/xenium/hpv/fold_masks/{anno_dict[slide]}.tif'
    ...
    # One slide is miscolored, so it messes up the fold mask transfer. Fix it here:
    if slide == '20250305__223640__X206_03052025_HPVTMA_01_02/output-XETG00206__0060364__Region_1__20250305__223715':
        arr_he[arr_he == 8] = 1
        arr_he[arr_he == 0] = 8
        arr_he[arr_he == 1] = 0
    ...
    moving_lv0 = sitk.GetImageFromArray(arr_he.astype('float32'))
    ...
    for batch in df_idx_cores[df_idx_cores['slide'] == slide].index:
        ...
        imgidx = df_idx_cores.loc[batch] * (2 ** 4)
        
        # Crop the core from the annotation mask
        arr_core_mask = nda[imgidx.y0:imgidx.y1, imgidx.x0:imgidx.x1,1]
        arr_core_mask[arr_core_mask != 0] = 1
        
        # Only do core-by-core alignment if there's something interesting in the mask (or the whole image is masked)
        if len(np.unique(arr_core_mask)) > 1 or np.array_equal(np.unique(arr_core_mask), [1]):
            ...
            np.save(f'/common/lamt2/HPV/data/xenium/alignment_v2/annotations/{batch.split('/')[1]}_foldmask.npy', img_arr)
        ...
        if core in manual_masks[slide]:
            print(f'applying manual mask to {core}')
            t4 = time.time()
            fixed_core = sitk.ReadImage(f'/common/lamt2/HPV/data/xenium/alignment_v2/prealigned_cores/{batch.split('/')[-1]}_dapi.png', sitk.sitkFloat32)
            fixed_arr = sitk.GetArrayViewFromImage(fixed_core)
            mask = np.ones((fixed_arr.shape[0], fixed_arr.shape[1])) * 255

            del fixed_core, fixed_arr
            gc.collect()
            
            np.save(f'/common/lamt2/HPV/data/xenium/alignment_v2/annotations/{batch.split('/')[-1]}_foldmask.npy', mask)
